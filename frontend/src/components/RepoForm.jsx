...
export default function RepoForm({ setRepoState, setChatContext }){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      setRepoState({ status: 'processing', repoName: null, files: 0, chunks: 0, error: null })

      const data = await analyzeRepository(url.trim())

      if (data?.success) {
        setRepoState({ status: 'success', repoName: data.repoName, files: data.files, chunks: data.chunks, error: null })
        setChatContext({ processed: true })
      } else {
        setRepoState({ status: 'error', repoName: null, files: 0, chunks: 0, error: data?.error || 'Unable to analyze repository' })
      }

    } catch (err) {
      console.error('Analyze error', err)
      setRepoState({ status: 'error', repoName: null, files: 0, chunks: 0, error: err.message || 'Network error' })
    } finally {
      setLoading(false)
    }
  }
